...
- Historical target, expected/input value, and timeout details in failed results are currently reconstructed from the latest saved test case. If the test changes after a run, those details can drift from the exact historical execution state.
