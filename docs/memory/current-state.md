...
The repository now has a working Electron + React + TypeScript MVP workbench with local project storage, manual step editing, test-level management, recorder, runner, results, and documented Windows x64 packaging. The packaged missing-Chromium path is validated on the current Windows portable build, so MVP product capability work is back on workflow polish. Final publisher/legal metadata remains deferred and does not block feature development.
...
Packaged missing-Chromium validation and test-level management are complete on the current Windows portable build. The next smallest valuable work is project-level management: add project rename and guarded delete or recovery flows without changing the accepted local schema. Final publisher/legal metadata remains deferred and should not block MVP feature work.
